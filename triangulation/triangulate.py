# wireframe_triangulate.py  -- cleaned multi-view pairwise triangulation
import cv2
import numpy as np
...
if len(img_ids) >= 2:
    ...
    I_a = cv2.imread(str(DATA_IMG_DIR / img_a["name"]))
    I_b = cv2.imread(str(DATA_IMG_DIR / img_b["name"]))
...
for id1, id2 in combinations(img_ids, 2):
    ...
    I1_pair = cv2.imread(str(DATA_IMG_DIR / img1["name"]))
    I2_pair = cv2.imread(str(DATA_IMG_DIR / img2["name"]))
    if I1_pair is None or I2_pair is None:
        print("Failed to load images:", img1["name"], img2["name"])
        continue
...
# สมมติว่า matched_pts3d กับ des ของ ORB อยู่แล้ว
# สำหรับ demo เราใช้ ORB descriptors ของ endpoints ที่ triangulate ได้

# --- สร้าง descriptors สำหรับ 3D points
orb = cv2.ORB_create(2000)
bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True)

# สมมติ matched_pts1, I1_color จากภาพคู่ที่ triangulate สุดท้าย
# หรือรวมจากทุกคู่
# เราจะใช้ dummy descriptors สำหรับ 3D points ตอนนี้
descriptors3d = np.random.randint(0,256,(all_3d_points.shape[0],32),dtype=np.uint8)

# บันทึก
np.save("triangulated_points.npy", all_3d_points)
np.save("triangulated_desc.npy", descriptors3d)
